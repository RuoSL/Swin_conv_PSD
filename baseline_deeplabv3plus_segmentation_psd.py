...
class ASPP(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, dilations: Tuple[int, int, int] = (6, 12, 18)):
        super().__init__()
        ...
        self.branch2 = ASPPConv(in_ch, out_ch, dilations[0])
        self.branch3 = ASPPConv(in_ch, out_ch, dilations[1])
        self.branch4 = ASPPConv(in_ch, out_ch, dilations[2])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        features = [self.branch1(x), self.branch2(x), self.branch3(x), self.branch4(x), pooled]
...
class DeepLabV3Plus(nn.Module):
    def __init__(self, in_ch: int = 3, out_ch: int = 1, base_ch: int = 32):
        ...
        self.encoder_out = DoubleConv(base_ch * 8, base_ch * 16)

        self.aspp = ASPP(base_ch * 16, base_ch * 8)
        self.low_project = nn.Sequential(
            nn.Conv2d(base_ch * 2, base_ch, kernel_size=1, bias=False),
            nn.BatchNorm2d(base_ch),
            nn.ReLU(inplace=True),
        )
        self.decoder = nn.Sequential(
            DoubleConv(base_ch * 9, base_ch * 4),
            nn.Dropout(0.1),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        e1 = self.enc1(x)
        low = self.enc2(self.pool(e1))
        e3 = self.enc3(self.pool(low))
        e4 = self.enc4(self.pool(e3))
        encoder_out = self.encoder_out(self.pool(e4))

        context = self.aspp(encoder_out)
...
def run(args: argparse.Namespace) -> None:
    ...
    out_dir = Path(args.output_dir) / "deeplabv3plus_segmentation_contour_psd" / run_id
    ...
    for fold_idx in range(args.splits):
        ...
        model = DeepLabV3Plus(base_ch=args.base_channels).to(device)
        ...
        ckpt_path = fold_dir / "best_deeplabv3plus.pt"
        history = []
        ...
        fold_record = {
            "fold": fold_idx + 1,
            "mode": "deeplabv3plus_segmentation_contour_psd",
            "test_area": test_area,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DeepLabV3+ segmentation baseline + contour PSD measurement.")
