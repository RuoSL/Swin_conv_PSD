...
class UNetPlusPlus(nn.Module):
    def __init__(self, in_ch: int = 3, out_ch: int = 1, base_ch: int = 32):
        ...
        self.conv0_0 = DoubleConv(in_ch, nb[0])
    ...
    @staticmethod
    def upsample(x: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
...
def run(args: argparse.Namespace) -> None:
    ...
    out_dir = Path(args.output_dir) / "unetpp_segmentation_contour_psd" / run_id
    ...
    for fold_idx in range(args.splits):
        ...
        model = UNetPlusPlus(base_ch=args.base_channels).to(device)
        ...
        fold_record = {
            "fold": fold_idx + 1,
            "mode": "unetpp_segmentation_contour_psd",
            "test_area": test_area,
